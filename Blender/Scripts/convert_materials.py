...
models = [obj for obj in bpy.context.scene.objects if obj.name.startswith("cbus")]
...
for mat in bpy.data.materials:
    ...
    for node in nodes:
        if node.type == "NORMAL_MAP":
            nodes.remove(node)
    nodes.get("Principled BSDF").inputs["Metallic"].default_value = 0
...
dupes = []
for obj in models:
    copies = split_copy_mesh(obj)
    for copy in copies:
        if copy.name == "buildings":
            copy.data.materials.append(bpy.data.materials["BuildingDupe.004"])
            for polygon in copy.data.polygons:
                polygon.material_index = mat_dict["BuildingDupe.004"]
            copy.active_material = bpy.data.materials["BuildingDupe.004"]
        else:
            copy.data.materials.append(bpy.data.materials["TerrainDupe.004"])
            for polygon in copy.data.polygons:
                polygon.material_index = mat_dict["TerrainDupe.004"]
        #bpy.data.collections["Dupes"].link(copy)
        copy.rotation_euler.x = np.deg2rad(90)
    dupes.extend(copies)

copy = split_copy_mesh(bpy.data.objects["models_0"])
